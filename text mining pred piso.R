...
tp2.work$piso <- ifelse(tp2.work$piso==">10","10",tp2.work$piso)
tp2.work$piso <- as.numeric(tp2.work$piso)
tp2.work$piso <- ifelse(tp2.work$piso>2,"3+",tp2.work$piso)
...
mat.df <- cbind(mat.df[,colSums(mat.df)>300], factor(tp2.work$piso))
save(dtm,file = "dtm")
remove(dtm)
